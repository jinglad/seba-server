...
const hospitalAppointmentModel = require("../models/hospitalAppointment.model");
...
const hospitalIpn = async (req, res) => {
  ...
  const tran_id = payment["tran_id"];
  if (payment["status"] === "VALID") {
    await hospitalAppointmentModel.findOneAndUpdate(
      { transactionId: tran_id },
      { status: "approved" },
      {
        new: true,
...
const intiPaymentHospital = async (req, res) => {
  let appointment = await hospitalAppointmentModel
    .find({
      user: req.user._id,
    })
    .sort({ createdAt: -1 })
  ...
  payment.setUrls({
    ...
    fail: "yoursite.com/fail", // If payment failed
    cancel: "yoursite.com/cancel", // If user cancel payment
    ipn: "https://seba-server.onrender.com/api/payment/hospital/ipn", // SSLCommerz will send http post request in this link
  });
